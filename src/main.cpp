...
bool fileTypeReached(string& input, int index);
//Returns true if the file's type extension follow the current index
...
int main()
{
  //Disable SFML error message dump
  sf::err().rdbuf(NULL);
  ...
  while (userInput != QUIT) {
    if (!parseCommand(userInput, cInfo)) {
      cInfo.reset();
      cout << endl << "Invalid Command" << endl;
    } else {
      if (!cInfo.executeCommands(sMod)) {
	cout << endl << "Something went wrong, please check your parameters."
	  << endl; 
      }
      cInfo.reset();
    }
...
bool parseCommand(string input, CommandInfo& cInfo) {
  ...
  if (input.find(CLEAR) == 0) {
    ...
    return true;
  } else if (input.find(START + "-") != 0 || 
	     input.length() < MIN_COMMAND_LENGTH) {
  ...
  while (index < input.length()) {
	...
	//Grab file name
	while (index < input.length() && !fileTypeReached(input, ++index))
